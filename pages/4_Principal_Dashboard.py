...
with tab1:
    ...
    required_cols = ['User Name', 'Class', 'Role', 'Row ID', 'Instructions', 'Gmail ID']
    if not all(col in df_users.columns for col in required_cols):
        st.error(f"Your ALL_USERS_SHEET is missing one or more required columns. Please ensure it has: {required_cols}")
    else:
        if message_type == "Private Instruction":
            with st.form("instruction_form"):
                df_temp = df_users.copy()
                
                # Create a display name that includes the class for students
                ...
                if st.form_submit_button("Send Instruction"):
                    if selected_display_name and instruction_text:
                        # Extract the real name from the selection before searching
